...
int main()
{
	...
	while(1)
	{
		...
		switch(cmd[0])
		{
			...
			case 'c': // rx beacon interaction
				close(fd);
				retro_activation();
				break;

			case 's': // rx beacon interaction
				close(fd);
				retro_activation();
				break;
...
void rx_cal_bd(int sock, struct sockaddr_in cliaddr, int addrlen)
{
	...
	{					
		...
		else if(able_status == 0x400)
		{
			printf("rx enable check\n");
			break;
		}
	}
	able_status == 0x0;
